...
if(isset($_SESSION['id'])) {
   ...
   if(isset($_POST['newpseudo']) AND !empty($_POST['newpseudo']) AND $_POST['newpseudo'] != $user['pseudo']) {
      ...
      $insertpseudo = $bdd->prepare("UPDATE vendeurs SET pseudo = ? WHERE id = ?");
      $insertpseudo->execute(array($newpseudo, $_SESSION['id']));
      header('Location: piscine_html.php?id='.$_SESSION['id']);
   }
   if(isset($_POST['newmail']) AND !empty($_POST['newmail']) AND $_POST['newmail'] != $user['mail']) {
      $newmail = htmlspecialchars($_POST['newmail']);
      $insertmail = $bdd->prepare("UPDATE vendeurs SET mail = ? WHERE id = ?");
   ...
   if(isset($_POST['newmdp1']) AND !empty($_POST['newmdp1']) AND isset($_POST['newmdp2']) AND !empty($_POST['newmdp2'])) {
      ...
      if($mdp1 == $mdp2) {
         $insertmdp = $bdd->prepare("UPDATE vendeurs SET motdepasse = ? WHERE id = ?");
